...
export async function downloadAllQRCodePostersPDF({
  items,
  venueLogoUrl,
  filename = "qrs",
}: Params) {
  if (!items.length) throw new Error("No hay QRs para descargar");

  const tappealoDataUrl = await imageToDataUrl(String(tappealoLogo));
  const mpDataUrl = await imageToDataUrl(String(MercadoPago));

  const logo =
    venueLogoUrl?.startsWith("data:image/")
      ? venueLogoUrl
      : null;
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const qrDataUrl = await QRCodeLib.toDataURL(item.qrUrl, {
      width: 900,
      margin: 1,
      color: {
        dark: "#3D1F12",
    ...
    wrapper.innerHTML = createPosterHTML({
      qrDataUrl,
      qrName: item.qrName,
      logo,
      tappealoDataUrl,
