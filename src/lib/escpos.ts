...
export const generateReceiptCommands = (order: Order, venueName: string): string[] => {
  ...
  order.items.forEach((item) => {
    commands.push(COMMANDS.BOLD_ON);
    commands.push(`${item.cantidad}x ${item.item}\n`);
    commands.push(COMMANDS.BOLD_OFF);
    
    if (item.descripcion && item.descripcion.trim()) {
      commands.push(`   ${item.descripcion}\n`);
    }
  });
  ...
  // Delivery info
  if (order.lugar_entrega) {
    commands.push(COMMANDS.BOLD_ON);
    commands.push('ENTREGA:\n');
    commands.push(COMMANDS.BOLD_OFF);
    commands.push(`${order.lugar_entrega}\n`);
    commands.push('\n');
  }
  
  // Customer info
  if (order.nombre) {
    commands.push(`Cliente: ${order.nombre}\n`);
  }
  
  if (order.telefono) {
    commands.push(`Tel: ${order.telefono}\n`);
  }
  ...
  // Comments
  if (order.comentarios_generales) {
    commands.push('\n');
    commands.push(COMMANDS.LINE);
    commands.push(COMMANDS.BOLD_ON);
    commands.push('NOTAS:\n');
    commands.push(COMMANDS.BOLD_OFF);
    commands.push(`${order.comentarios_generales}\n`);
  }
